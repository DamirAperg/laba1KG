...
for l in _f:
    ...
    x_max = min(1999, max(x0, x1, x2))
    y_min = max(0, min(y0, y1, y2))
    y_max = min(1999, max(y0, y1, y2))
    ...
    if (skalar/(n_norm*light_norm) < 0):
        for x in range(int(x_min), int(x_max)+1):
            for y in range(int(y_min), int(y_max)+1):
                lambda0 = baricentr0(x, y, x0, y0, x1, y1, x2, y2)
                lambda1 = baricentr1(x, y, x0, y0, x1, y1, x2, y2)
                lambda2 = baricentr2(x, y, x0, y0, x1, y1, x2, y2)
